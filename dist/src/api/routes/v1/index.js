...
var _salesforceUtils = require('../../../helpers/salesforceUtils');
...
router.get('/login-salesforce', async (req, res, next) => {
	try {
		let login = await (0, _salesforceUtils.loginSalesforce)();
		res.status(200).json({
			status: 'success',
			payload: login
		});
...
router.get('/test-salesforce', async (req, res, next) => {
	try {
		let isLoggedIn = await (0, _salesforceUtils.checkIfSessionExist)();
		res.status(200).json({
			status: 'success',
			payload: isLoggedIn
		});
